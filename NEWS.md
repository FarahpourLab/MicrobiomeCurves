# MicrobiomeCurves 0.99.0

First submission to Bioconductor.

## Features

* `mc_run()` takes data in the form a study produces: an abundance table
  ...
  on, with no assumption of species. `mc_from_metadata()` performs the
...
* `mc_uncertainty()` returns the same intervals as a table, for filtering a
...
* `mc_run()` is a generic with methods for `data.frame` and for
...
* `mc_detect_missing()` reports which samples are missing and why, without
  fitting a model.
* `mc_prepare()`, `mc_fit()`, `mc_impute()` and `mc_metrics()` provide the
  benchmarking workflow, in which observed values are masked and then scored.
* `mc_ci()` returns analytic and bootstrap confidence intervals for an
  imputed value. `mc_plot()` draws a trajectory with its interval.
...
* Fits that fall back or fail are reported once at the end of a run, rather
  ...
  on three or fewer curves, and functional k-means fallbacks. `mc_metrics()`
...
* `taxa_demo` provides a small simulated dataset containing both forms of
  missingness. `mc_as_demo_se()` returns the same data as a
...
* `mc_plot()` no longer requires **dplyr** and **patchwork** to be attached
