...
# fit. Everything printed here is also returned in the mc_design object, so
# nothing is only available by reading the console.

#' Report a design as it is built
#'
#' @param design An object of class `mc_design`.
...
#' @keywords internal
#' @noRd
mc_report_design <- function(design, say) {
    ...
    say("  time points, in order: ", mc_axis_text(design$axis))

    mc_report_gaps(design, n_cells, say)
    mc_report_thin(design, say)
    invisible(NULL)
}
...
#' @keywords internal
#' @noRd
mc_report_gaps <- function(design, n_cells, say) {
    miss <- design$missing
    if (nrow(miss) == 0) {
    ...
    for (r in c("absent_sample", "no_data")) {
        ...
        if (nrow(sel) == 0) next
        say(
            "    ", nrow(sel), " ", mc_reason_text(r), ": ",
            mc_fmt_some(paste0(sel$subject, " at ", sel$time_label), n = 6)
        )
    }
    invisible(NULL)
}
...
#' @keywords internal
#' @noRd
mc_reason_text <- function(reason) {
    switch(reason,
...
#' @keywords internal
#' @noRd
mc_report_thin <- function(design, say) {
    obs <- design$observed
...
#' Print a design
#'
#' @param x An object of class `mc_design`.
#' @param ... Ignored.
#'
#' @return `x`, invisibly.
#'
...
#'
#' @export
print.mc_design <- function(x, ...) {
    cat("MicrobiomeCurves design\n")
    cat("  taxa        :", nrow(x$table), "\n")
    cat("  samples     :", nrow(x$map), "\n")
    cat("  subjects    :", length(x$subjects), "\n")
    cat("  time points :", mc_axis_text(x$axis), "\n")
    cat(
