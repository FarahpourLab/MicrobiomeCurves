...
#' @param design The `mc_design`, for the caller's own names.
#' @param use_outliers Logical. Whether the fit screened for outliers.
#'
#' @return A list of data frames and labels for [mc_uncertainty_panel()], or
#'   `NULL` when no band could be produced.
#'
#' @keywords internal
#' @noRd
mc_panel_data <- function(fit, species_name, rep_id, time_id, design,
                           use_outliers) {
    ...
    bands <- mc_panel_bands(fit, W_sp, rep_id, time_id, flagged)
    if (is.null(bands)) {
        return(NULL)
    }
    ...
    target <- subj(rep_id)
    tlab <- mc_time_label(design$axis, design$times[time_id + 1L])

    list(
        bands = bands,
        traj = mc_panel_traj(W_sp, design, flagged, use_outliers),
        observed = mc_panel_observed(W_sp, rep_id, design),
        imputed = mc_panel_imputed(fit, species_name, rep_id, time_id,
                                    design),
        truth = mc_panel_truth(fit, species_name, rep_id, time_id, design),
        target_time = time_id,
        cell_label = paste0(target, " at ", tlab),
        axis_breaks = seq_along(design$times) - 1L,
        axis_labels = mc_time_label(design$axis, design$times),
        title = paste0(species_name, ": ", target, " at time ", tlab),
        subtitle = mc_panel_subtitle(flagged, subj, use_outliers),
...
#' @keywords internal
#' @noRd
mc_panel_subtitle <- function(flagged, subj, use_outliers) {
