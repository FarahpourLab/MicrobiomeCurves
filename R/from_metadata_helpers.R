# Validation and assembly for mc_from_metadata().
#
...
#' @keywords internal
#' @noRd
mc_abundance_parts <- function(abundance) {
    ...
    taxa <- mc_abundance_taxa(abundance)
    ...
    if (length(bad) > 0) {
        stop(
            "These abundance columns are not numeric: ", mc_fmt_some(bad),
...
#' @keywords internal
#' @noRd
mc_abundance_taxa <- function(abundance) {
    ...
    if (anyDuplicated(rn) > 0) {
        stop(
            ...
            mc_fmt_some(unique(rn[duplicated(rn)])),
...
#' @keywords internal
#' @noRd
mc_metadata_parts <- function(metadata, sample_col, subject_col, time_col,
                               ab_samples) {
    ...
    mc_check_meta_cols(metadata, sample_col, subject_col, time_col)

    sample <- as.character(metadata[[sample_col]])
    subject <- as.character(metadata[[subject_col]])
    axis <- mc_time_axis(metadata[[time_col]], time_col)

    mc_check_meta_values(sample, subject, sample_col, subject_col)
    mc_check_sample_overlap(sample, ab_samples, sample_col)
...
#' @keywords internal
#' @noRd
mc_check_meta_cols <- function(metadata, sample_col, subject_col, time_col) {
    named <- c(
    ...
    if (length(missing) > 0) {
        quoted <- mc_fmt_some(paste0("'", missing, "'"))
        by <- mc_fmt_some(names(missing))
        have <- mc_fmt_some(names(metadata))
...
#' @keywords internal
#' @noRd
mc_check_meta_values <- function(sample, subject, sample_col, subject_col) {
    ...
    if (length(dup) > 0) {
        stop(
            ...
            mc_fmt_some(dup), ". Each sample needs exactly one row.",
            call. = FALSE
        )
    }
    invisible(NULL)
...
#' @keywords internal
#' @noRd
mc_check_sample_overlap <- function(meta_samples, ab_samples, sample_col) {
    ...
    if (length(shared) == 0) {
        stop(
            ...
            mc_fmt_some(meta_samples), "; abundance has ",
            mc_fmt_some(ab_samples),
    ...
    if (length(undescribed) > 0) {
        stop(
            ...
            "the metadata: ", mc_fmt_some(undescribed),
