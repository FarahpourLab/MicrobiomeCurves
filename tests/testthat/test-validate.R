...
test_that("a mask that matches nothing says what did not match", {
    ...
    expect_error(
        tti_prepare(b$dat, "OTU_ID", ghost),
...
test_that("masking every sample is refused", {
    ...
    expect_error(
        tti_prepare(b$dat, "OTU_ID", mask_matrix = all_masked),
...
test_that("mask_list rejects elements that are not pairs", {
    ...
    expect_error(
        tti_prepare(b$dat, "OTU_ID", mask_list = trap),
...
test_that("the documented mask_list forms both work", {
    b <- demo_bits()
    from_list <- tti_prepare(b$dat, "OTU_ID",
        mask_list = list(c(b$subs[1], b$times[2]))
    )
    from_df <- tti_prepare(b$dat, "OTU_ID", b$mask)
...
test_that("tti_detect_missing reports unusable columns", {
    b <- demo_bits()
    junk <- b$dat
    junk$free_text <- "note"
    expect_warning(
        tti_detect_missing(junk, "OTU_ID"),
        "do not match"
    )
})
...
test_that("a fully NA sample is still fine, and is imputed", {
    ...
    expect_no_error(tti_detect_missing(whole, "OTU_ID"))
})

test_that("kmeans_fd is a real alternative, not an alias", {
    b <- demo_bits()
    prep <- tti_prepare(b$dat, "OTU_ID", b$mask)
    f_fpca <- suppressWarnings(tti_fit(prep, K = 2))
    f_kfd <- suppressWarnings(
        tti_fit(prep, K = 2, cluster_method = "kmeans_fd")
    )
...
test_that("an unknown cluster_method is rejected", {
    ...
    prep <- tti_prepare(b$dat, "OTU_ID", b$mask)
    expect_error(
        tti_fit(prep, K = 2, cluster_method = "nope"),
        "should be one of"
    )
})
