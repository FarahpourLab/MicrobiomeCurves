test_that("tti_run fills every missing cell and reports what it did", {
...
test_that("tti_run refuses a table with nothing missing", {
...
test_that("tti_run warns about subjects with too few observations", {
...
test_that("fitting survives a subject with too few observations", {
    ...
    #     subject FPCA could use, while tti_fit() named it against ALL subjects
    #     ("'names' attribute [4] must be the same length as the vector [3]").
    #
    #   either mode -> with no observations left for the target subject,
    #     tti_analytic_ci() built obs_idx via sapply() over an empty vector,
    ...
    # < 2 observations to fully observed. tti_run() does not, so users can.
    set.seed(1)
    dat <- data.frame(OTU_ID = c("T1", "T2", "T3"))
    for (s in c("S1", "S2", "S3", "S4")) {
        for (tt in 0:3) dat[[paste0(s, ".", tt)]] <- rnorm(3)
    ...
    for (uo in c(TRUE, FALSE)) {
        fit <- expect_no_error(
            suppressWarnings(tti_fit(prep, K = 1, use_outliers = uo))
        )
        got <- tti_impute(fit)
...
test_that("tti_run adds no logic of its own", {
...
test_that("print.tti_run returns its input invisibly", {
    run <- quiet_run(taxa_demo, taxon_col = "OTU_ID", K = 1)

    expect_output(print(run), "TaxaTimeImpute run")
