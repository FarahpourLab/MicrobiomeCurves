...
def fc_bn(_x, _output, _phase, _scope):
    with tf.variable_scope(_scope):
        h1 = tf.contrib.layers.fully_connected(_x, _output, activation_fn=None, scope='dense',
                                               weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                               weights_regularizer=tf.contrib.layers.l2_regularizer(0.01),
                                               reuse=tf.AUTO_REUSE)
        h2 = tf.contrib.layers.batch_norm(h1, updates_collections=None, fused=True, decay=0.9, center=True, scale=True,
                                          is_training=_phase, scope='bn', reuse=tf.AUTO_REUSE)
        return h2
...
profiles_file_path = sys.argv[1]
mask_file_path = sys.argv[2]
output_dir = sys.argv[3]

learning_rate_input = float(sys.argv[4])
dropout_rate_input = float(sys.argv[5])
epochs_input = int(sys.argv[6])

profiles_file_name = os.path.basename(profiles_file_path).split('.csv')[0]

pseudo_count = float(profiles_file_name.split('_')[-1])
mask_file_name = '.'.join(os.path.basename(mask_file_path).split('.csv')[0].split('.')[1:])

os.makedirs(f"{output_dir}/", exist_ok=True)

x_data = pd.read_csv(profiles_file_path, index_col=0)  # Row: each sample, Column: Features
...
mask_data = pd.read_csv(mask_file_path, index_col=0)  # Row: sample, Column: timepoint
...
def cnn_layer(_x, _num_feature, _scope, _keep_prob):
    with tf.variable_scope(_scope):
        cnn_1 = tf.nn.dropout(
            tf.keras.layers.Conv1D(16, 3, strides=1, padding='same', input_shape=(num_timepoint, _num_feature),
                                   activation=tf.nn.leaky_relu)(_x), _keep_prob)
        cnn_1_maxpool = tf.keras.layers.MaxPool1D(pool_size=1, strides=1, padding='same')(cnn_1)
        cnn_2 = tf.nn.dropout(tf.keras.layers.Conv1D(8, 3, strides=1, padding='same', input_shape=(num_timepoint, 16),
                                                     activation=tf.nn.leaky_relu)(cnn_1_maxpool), _keep_prob)
...
def build_generator(input_x, cluster_x, mask, timedecay_x_f, timedecay_x_b, _phase, _keep_prob):
    for i in range(len(cluster_x)):
        if i == 0:
            cluster_cnn_feature = cnn_layer(cluster_x[i], cluster_feature_num_list[i], "cnn_cluster_" + str(i),
                                            _keep_prob)
        else:
    ...
    rnn_f, rnn_b = tf.keras.layers.Bidirectional(tf.keras.layers.SimpleRNN(10, return_sequences=True), merge_mode=None)(
        cluster_cnn_feature)
...
generated_x, imputed_x, x_real_mask, x_estimated_mask, difference_fb = build_generator(tf_X, tf_X_cluster_list, tf_mask,
                                                                                       tf_timeDecay_f, tf_timeDecay_b,
                                                                                       phase_gen, keep_prob)
...
gen_feed_dict = {tf_X: x, tf_mask: mask_vector, tf_timeDecay_f: timeDecay_forward, tf_timeDecay_b: timeDecay_backward,
                 phase_discriminator: False, phase_gen: True, keep_prob: keep_rate, tf_Y_time: time_label}
for j in range(len(cluster_df_list)):
    gen_feed_dict[tf_X_cluster_list[j]] = cluster_df_list[j]

imp_feed_dict = {tf_X: x, tf_mask: mask_vector, tf_timeDecay_f: timeDecay_forward, tf_timeDecay_b: timeDecay_backward,
                 phase_discriminator: False, phase_gen: False, keep_prob: 1.0, tf_Y_time: time_label}
for j in range(len(cluster_df_list)):
...
with tf.Session() as sess:
    ...
    for i in range(num_epoch):
        for j in range(num_discriminator_epoch):
            ...
            disc_feed_dict = {tf_X: imputed_dataset, tf_mask: mask_vector, tf_timeDecay_f: timeDecay_forward,
                              tf_timeDecay_b: timeDecay_backward, phase_discriminator: True, phase_gen: False,
                              keep_prob: 1.0, tf_Y_time: time_label}
        ...
        for j in range(num_generator_epoch):
            imputed_dataset = sess.run(imputed_x, feed_dict=imp_feed_dict)
            gen_feed_dict = {tf_X: imputed_dataset, tf_mask: mask_vector, tf_timeDecay_f: timeDecay_forward,
                             tf_timeDecay_b: timeDecay_backward, phase_discriminator: False, phase_gen: True,
                             keep_prob: keep_rate, tf_Y_time: time_label}
            for t in range(len(cluster_df_list)):
                gen_feed_dict[tf_X_cluster_list[t]] = cluster_df_list[t]
            _, g_loss_print, r_loss, c_loss = sess.run(
                [g_train_step, g_loss, masked_reconstruction_loss, consistency_loss], feed_dict=gen_feed_dict)
        if i % 10 == 0:
            print('Epoch: %d, g_loss: %f, r_loss:%f, c_loss: %f, d_loss: %f' % (
            i, g_loss_print, r_loss, c_loss, d_loss_print))
            if min > r_loss:
                min = r_loss
                stop_count = 0
    ...
    imputed_dataset_reshape_df.to_csv(f"{output_dir}/imputed_{profiles_file_name}_{mask_file_name}.csv",
                                      index=True)
    ...
    imputed_dataset_reshape_df_scaled.to_csv(f"{output_dir}/imputed_{profiles_file_name}_{mask_file_name}_scaled.csv",
                                             index=True)
