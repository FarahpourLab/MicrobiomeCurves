# BAMITA dependencies. The functions file (Bayesian_Tensor_Imputation_Functions.R)
# loads no packages itself, so every library it relies on must be attached here.
library(rTensor)
library(tensor)
library(multiway)
library(MASS)
...
# ---------------------------------------------------------------------------
# BAMITA for the DIABIMMUNE "three country" dataset.
# ONE COUNTRY per job (passed as argument: EST / FIN / RUS); all three
# missingness mechanisms (MCAR, MAR, MNAR) are processed for that country.
# No parameter tuning: fixed num_com / max / burn.in / thin (tutorial i.i.d.
# settings). This script lives 3 levels under the repo root.
# ---------------------------------------------------------------------------

args <- commandArgs(trailingOnly = TRUE)
country <- if (length(args) >= 1) args[1] else stop("usage: Rscript run_bamita_diabimmune.R <COUNTRY>")
cat(sprintf("Running BAMITA for country: %s\n", country))
...
# Match the upstream tutorial's i.i.d. example exactly:
#   numc = 3, max = 5000, burn.in = 2000, thin = 1, cpem_as_initial = FALSE.
num_com <- 3
max_iter <- 5000
burn_in <- 2000
thin <- 1
...
parse_condition <- function(file_name) {
  parts <- strsplit(sub("^sorted_clr_transformation_relative_abundance_", "", file_name), "_", fixed = TRUE)[[1]]
  parts[3]
}
...
# One BAMITA fit for a single mask file. mechanism + output_dir are passed in
# (they change per mechanism), so a single bad fit cannot abort the whole run.
process_mask <- function(mask_file, truth_tensor, tensor_info, truth_df,
                         file_name, tax_level, condition, pseudo_count,
                         mechanism, output_dir) {
  tryCatch({
...
# the single sorted input file for this country
input_file <- list.files(
  input_dir,
  pattern = paste0("^sorted_clr_transformation_relative_abundance_", tax_level_expected, "_", country, "_.*\\.csv$"),
  full.names = TRUE
)
if (length(input_file) == 0) stop(sprintf("no sorted input found for country '%s' in %s", country, input_dir))
input_file <- input_file[1]

file_name <- basename(input_file)
tax_level <- parse_tax_level(file_name)
condition <- parse_condition(file_name)
...
for (mechanism in mechanisms) {
  ...
  mask_files <- list.files(
    mask_dir,
    pattern = paste0("masked_metadata_", tax_level, "_", condition, "\\.rep_.*\\.csv$"),
    full.names = TRUE, recursive = TRUE
  )

  cat(sprintf("[%s/%s] %s: %d mask files across %d core(s)\n",
              mechanism, condition, file_name, length(mask_files), n_cores))
  ...
  if (length(failed) > 0) {
    warning(sprintf("[%s/%s] %d of %d mask fits failed", mechanism, condition,
                    length(failed), length(mask_files)))
