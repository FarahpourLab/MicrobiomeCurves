"""Sort a CLR table into the DeepMicroGen input layout (CLI helper).

Usage:
    python convert_input.py <clr_table.csv> <out_dir>

Groups OTUs by phylum (largest phylum first), and within each phylum orders OTUs
by their mean absolute Spearman correlation with the others. Each phylum becomes
a ``cluster`` (integer id appended as a column) -- the grouping DeepMicroGen's
CNN expects. Writes ``<out_dir>/sorted_<input_name>.csv``.
"""
import pandas as pd
import os
import sys
...
for i in range(len(phylum_otu_count_df)):
    ...
    tmp_phylum_otu_corr_df = pd.DataFrame({'#OTU ID': tmp_phylum_otu_list, 'corr': tmp_phylum_otu_corr_list})
    tmp_phylum_otu_corr_df = tmp_phylum_otu_corr_df.sort_values(by='corr', ascending=False)
    tmp_data_based_on_corr = pd.merge(tmp_phylum_otu_corr_df, data, left_on="#OTU ID", right_on=otu_column_name)
...
final_data_df.to_csv(f'{out_dir}/sorted_{file_name}.csv', mode="w", index=False)
