"""MCAR (Missing Completely At Random) mask generation for DIABIMMUNE.

For each country the subject x timepoint presence matrix is rebuilt from the
shared relative-abundance columns and a fraction of cells is masked uniformly at
random (independent of value or time) -> MCAR.

Input
    03.processed/02.relative_abundance/relative_abundance_l7_species_<country>.csv
    (``#OTU ID`` rows + ``subjectID.tp`` columns), written by
    02.preparation/common_timepoints_data_prep.py.
Output
    03.processed/MCAR/mask/l7_species/<ratio>/
    masked_metadata_l7_species_<country>.rep_<rep>.seed_<seed>.csv
    (subject x timepoint 0/1 matrix; 0 = masked).
"""
import pandas as pd
import numpy as np
import os
...
def split_subject_tp(columns):
    """Sample columns look like 'E002338.0' -> subject 'E002338', timepoint 0."""
...
def build_presence(columns):
    """Subject x timepoint matrix (1 where a sample exists) from abundance columns."""
...
def random_mask(presence, subject_col, out_dir, file_name, n_reps=N_REPS, ratios=RATIOS):
    """MCAR: for each ratio mask k = ratio * n_cells uniformly random cells (-> 0)."""
...
if __name__ == "__main__":
    ...
    # start clean: mask filenames carry random seeds, so reruns would otherwise pile up
    if os.path.isdir(mask_root):
        shutil.rmtree(mask_root)
    pattern = os.path.join(INPUT_DIR, f"relative_abundance_{TAX_LEVEL}_*.csv")

    for path in sorted(glob.glob(pattern)):
        country = os.path.basename(path)[len(f"relative_abundance_{TAX_LEVEL}_"):-len(".csv")]
